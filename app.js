...
function show(view){[authView,verifyView,lobbyView,gameView].forEach(v=>v.classList.add('hidden'));view.classList.remove('hidden');}
...
function fakeLoad(){const saved = localStorage.getItem('runastar_session'); if(saved){const session = JSON.parse(saved); syncLobby(session); show(lobbyView); return true;} return false;}
function saveSession(session){ localStorage.setItem('runastar_session', JSON.stringify(session)); syncLobby(session); }

function startGame(){
  game = { x: 160, y: 270, vx: 0, vy: 0, hp: 100, ammo: 6, enemies: 3, bullets: [], lastShot: 0, dashCooldown: 0, skillCooldown: 0, respawnTimer: 0 };
  roomState.tick = 0;
  roomState.allies = 3;
  roomState.enemies = 3;
  hpValue.textContent = game.hp;
  ammoValue.textContent = game.ammo;
  enemyValue.textContent = game.enemies;
  show(gameView);
  requestAnimationFrame(loop);
}

function backToLobby(){ show(lobbyView); }

window.addEventListener('keydown', e => keys.add(e.key.toLowerCase()));
window.addEventListener('keyup', e => keys.delete(e.key.toLowerCase()));

function shoot(){
  if(!game || game.ammo <= 0 || Date.now() - game.lastShot < 180) return;
  game.ammo -= 1;
  game.lastShot = Date.now();
  ammoValue.textContent = game.ammo;
  game.bullets.push({ x: game.x + 26, y: game.y + 18, vx: 10, vy: 0 });
}

function dash(){
  if(!game || game.dashCooldown > 0) return;
  game.x = Math.min(canvas.width - 60, game.x + 80);
  game.dashCooldown = 60;
}

function skill(){
  if(!game || game.skillCooldown > 0) return;
  game.skillCooldown = 180;
  game.enemies = Math.max(0, game.enemies - 1);
  enemyValue.textContent = game.enemies;
}
...
function update(){
  ...
  roomState.tick += 1;
  if(roomState.tick % 120 === 0 && game.enemies > 0) {
    game.enemies -= 1;
    enemyValue.textContent = game.enemies;
  }

  if(game.enemies <= 0) {
    const session = JSON.parse(localStorage.getItem('runastar_session') || '{}');
    session.xp = (session.xp || 0) + 120;
...
function draw(){
  ...
  // map objects
  ctx.fillStyle = '#2d3f72';
  ctx.fillRect(250, 120, 120, 36);
  ctx.fillRect(480, 380, 160, 34);
  ctx.fillRect(610, 170, 60, 100);

  // players
  ctx.fillStyle = '#7c8cff';
  ctx.fillRect(game.x, game.y, 32, 32);
  ctx.fillStyle = '#66ffb0';
  ctx.fillRect(120, 120, 28, 28);
  ctx.fillRect(120, 240, 28, 28);
  ctx.fillRect(120, 360, 28, 28);

  // enemies
  ctx.fillStyle = '#ff7c7c';
  for(let i=0;i<game.enemies;i++) ctx.fillRect(700 + i*70, 120 + (i%2)*90, 28, 28);

  // bullets
  ctx.fillStyle = '#ffd56a';
  game.bullets.forEach(b => ctx.fillRect(b.x, b.y, 10, 4));

  // basic HUD text
  ctx.fillStyle = '#e9eeff';
  ctx.font = '18px system-ui';
  ctx.fillText(`Room sync: ${roomState.online ? 'online' : 'offline'}`, 24, 34);
  ctx.fillText(`Allies: ${roomState.allies}  Enemies: ${game.enemies}`, 24, 60);
}
...
startMatch.addEventListener('click', startGame);
